...
const DrLuciaScreen = ({navigation}) => {
    ...
    const getid = async() =>{
        try{
            const response = await getCategoyIdBySlug("dr-lucia-gannon");
    ...
    return(
        <SafeAreaView style={{ flex: 1, paddingTop: 5 }}>
          <Text style={styles.pageTitle}>Dr Lucia Gannon</Text>
      {data.length > 0 ? (
        <FlatList
          onEndReached={() => {
            if (!loading && !lastPage) {
