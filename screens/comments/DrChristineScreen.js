...
const DrChristineScreen = ({navigation}) => {
    ...
    const getid = async() =>{
        try{
            const response = await getCategoyIdBySlug("dr-christine-omalley");
    ...
    return(
        <SafeAreaView style={{ flex: 1, paddingTop: 5 }}>
          <Text style={styles.pageTitle}>Dr Christine O Malley</Text>
      {data.length > 0 ? (
        <FlatList
          onEndReached={() => {
            if (!loading && !lastPage) {
...
export default DrChristineScreen;
