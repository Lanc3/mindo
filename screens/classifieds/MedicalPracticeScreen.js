...
const MedicalPracticeScreen = ({navigation}) => {
    ...
    const getid = async() =>{
        try{
            const response = await getCategoyIdBySlug("medical-practice-for-sale-to-let");
    ...
    return(
        <SafeAreaView style={{ flex: 1, paddingTop: 5 }}>
          <Text style={styles.pageTitle}>Medical Practice - For Sale/To Let</Text>
      {data.length > 0 ? (
        <FlatList
          onEndReached={() => {
            if (!loading && !lastPage) {
