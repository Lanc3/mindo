...
import PodcastWebView from '../../components/PodcastWebView'
import SaveButton from '../../components/SaveFavoriteButton'
export default function PodcastReader({ props, route }) {
  ...
  const podcastData = content.podcastData
  const navigation = useNavigation()
  const decodeString = (str) => {
    return str.replace(/(&nbsp;|<([^>]+)>)/gi, '').replace(/^(-)+|(-)+$/g, '')
  }
  console.log(content.podcastData)
  const hide = () => {
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View>
            ...
            <View
              style={{ height: iframeHeight ,marginHorizontal:10}}
              renderToHardwareTextureAndroid={true}
            >
              <PodcastWebView
                callback={() => {
