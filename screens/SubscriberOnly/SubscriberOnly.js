...
const SubscriberOnly = ({navigation}) => {
    const [data, setData] = useState([]);
    const [page, setPage] = useState(1);
    const [loading, setLoading] = useState(0);
    const [totalPages, setTotalPages] = useState(0);
    const [title,setTitle] = useState("Subscriber Only");
    const [slug,setSlug] = useState("subscriber-only");
    ...
    return(
      ...
      {data.length > 0 ? (
        <View>
        <FlatList
          ListHeaderComponent={<Header title={title} adType={"LDB_MOBILE"} adType={"LDB_MOBILE"} navigation={navigation} data={data}></Header>}
          ListFooterComponent={
            <View>
