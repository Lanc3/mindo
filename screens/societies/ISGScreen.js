...
const ISGScreen = ({navigation}) => {
    ...
    return(
      ...
      {data.length > 0 ? (
        <View>
        <FlatList
          ListHeaderComponent={<Header title={title} blurb={blurb} adType={"ISG_LBD"} navigation={navigation} data={data}></Header>}
          ListFooterComponent={
            <View>
          ...
          <Footer navi={navigation} refS={scrollRef} adSelected="ISG_MPU"/>
            </View>
          }
          data={data}
          ...
          renderItem={({ item, index })=>{
            if(index === 3){
                return(<AdManager selectedAd={"ISG_MPU"} sizeType={"BIG"}/>)
            }
            else if(index === 7){
              return(<AdManager selectedAd={"ISG_LBD"} sizeType={"SMALL"}/>)
            }
            return(
