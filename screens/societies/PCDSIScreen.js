...
const PCDSIScreen = ({navigation}) => {
    const [data, setData] = useState([]);
    const [page, setPage] = useState(1);
    const [loading, setLoading] = useState(0);
    const [totalPages, setTotalPages] = useState(0);
    const [title,setTitle] = useState("Primary Care Dermatology Society");
    const [blurb,setBlurb] = useState("");
    const [slug,setSlug] = useState("pcdsi");
    ...
    return(
      ...
      {data.length > 0 ? (
        <View>
        <FlatList
          ListHeaderComponent={<Header title={title} blurb={blurb} adType={"PCDSI_LDB"} navigation={navigation} data={data}></Header>}
          ListFooterComponent={
            <View>
          ...
          <Footer navi={navigation} refS={scrollRef} adSelected="PCDSI_MPU"/>
            </View>
          }
          data={data}
          ...
          renderItem={({ item, index })=>{
            if(index === 3){
                return(<AdManager selectedAd={"PCDSI_MPU"} sizeType={"BIG"}/>)
            }
            else if(index === 7){
              return(<AdManager selectedAd={"PCDSI_LDB"} sizeType={"SMALL"}/>)
            }
            return(
