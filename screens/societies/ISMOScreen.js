...
const ISMOScreen = ({navigation}) => {
    const [data, setData] = useState([]);
    const [page, setPage] = useState(1);
    const [loading, setLoading] = useState(0);
    const [totalPages, setTotalPages] = useState(0);
    const [title,setTitle] = useState("The Irish Society of Medical Oncology");
    const [slug,setSlug] = useState("ismo");
    ...
    return(
      ...
      {data.length > 0 ? (
        <View>
        <FlatList
          ListHeaderComponent={<Header title={title} blurb={blurb}  adType={"ISMO_LBD"} navigation={navigation} data={data}></Header>}
          ListFooterComponent={
            <View>
          ...
          <Footer navi={navigation} refS={scrollRef} adSelected="ISMO_MPU"/>
            </View>
          }
          data={data}
          ...
          renderItem={({ item, index })=>{
            if(index === 3){
                return(<AdManager selectedAd={"ISMO_MPU"} sizeType={"BIG"}/>)
            }
            else if(index === 7){
              return(<AdManager selectedAd={"ISMO_LBD"} sizeType={"SMALL"}/>)
            }
            return(
