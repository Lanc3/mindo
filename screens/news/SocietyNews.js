...
const SocietyNews = ({navigation}) => {
    ...
    const getid = async() =>{
        try{
            const response = await getCategoyIdBySlug("society-news");
            const id = await response;
            const json = JSON.parse(await getPostsByCategory(id,page));
            const total = fetchApiData("society-news");//getting total pages per slug
    ...
    return(
        <ScrollView style={{ flex: 1, paddingTop: 5 }}>
          <Text style={styles.pageTitle}>Society News</Text>
          <Carousel
        style='slide'
        items={data}
        navigation={navigation}
        nameSlug={"Society News"}
      />
      {data.length > 0 ? (
        <View>
