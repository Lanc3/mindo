...
const Investigations = ({navigation}) => {
    ...
    const getid = async() =>{
        try{
            const response = await getCategoyIdBySlug("investigations");
            const id = await response;
            const json = JSON.parse(await getPostsByCategory(id,page));
            const total = fetchApiData("investigations");//getting total pages per slug
    ...
    return(
        <ScrollView style={{ flex: 1, paddingTop: 5 }}>
          <Text style={styles.pageTitle}>Investigations</Text>
          <Carousel
        style='slide'
        items={data}
        navigation={navigation}
        nameSlug={"Investigations"}
      />
      {data.length > 0 ? (
        <View>
